    ...
    if request.method == 'POST':
        try:
            if user_choice == 'Search Dog':
                ...
                # func.lower() converts searched value to lower before comparison
                ...
                get_user = db.session.query(Dog).filter(Dog.user.has(func.lower(User.user) ==
                                                                     func.lower(user_name))).all()
